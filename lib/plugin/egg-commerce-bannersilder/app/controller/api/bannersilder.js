...
let BannersilderController = {
    ...
    /**
     * @api {get} /api/bannersilder/getOne 查找指定位置 banner
     * @apiDescription 根据ID获取广告(单图或轮播图),包含后台自定义的名称 注意：返回结果中如果 state=true 才显示
     * @apiName /bannersilder/getOne
     * @apiGroup Bannersilder
     * @apiParam {string} name 广告位名称（anli/案例）
     * @apiSuccess {json} result
     * @apiSuccessExample {json} Success-Response:
     *  {
     *      "status": 200,
     *      "message": "bannersilder",
     ...
     *      }
     *  }
     * @apiSampleRequest http://localhost:8080/api/bannersilder/getOne
     * @apiVersion 1.0.0
     */
