...
let CategoryController = {
    ...
    /**
     * @api {get} /api/category/getList 获取文档类别列表
     * @apiDescription 获取文档类别列表
     * @apiName /category/getList
     * @apiGroup Category
     ...
     *    ]
     *}
     * @apiSampleRequest http://localhost:8080/api/category/getList
     * @apiVersion 1.0.0
     */
    async list(ctx, app) {
        ...
        try {
            ...
            let categoryList = await ctx.service.category.find({
                isPaging: '0'
            }, {
                query: queryObj
            });
    ...
    async treelist(ctx, app) {
        ...
        // let rootCate = _.filter(categoryList,(item)=>{
    ...
    async getCurrentCategoriesById(ctx, app) {
        ...
        try {
            ...
            if (typeId || !_.isEmpty(contentObj)) {
                let fullNav = await ctx.service.category.find({
                    isPaging: '0'
                });
